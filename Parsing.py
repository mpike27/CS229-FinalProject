...
full_broad_urls = [("WolvesArsenal042419", "https://www.youtube.com/watch?v=DWYKhbawzQ8")]
...
def main():
    for video_name, url in highlight_urls:
        if vs.download_from_url(url, video_name, train_path):
    ...
    for video_name, url in full_broad_urls:
        if vs.download_from_url(url, video_name, test_path):
            vs.parseVideo(test_path + video_name + '.mp4', clip_len)
